...
void setup()
{
  ...
  can.begin(STD_ID_LEN, BR250K, PORTB_8_9_XCVR); // 11b IDs, 250k bit rate, no transceiver chip, portA pins 11,12
  //can.begin(STD_ID_LEN, BR250K, PORTA_11_12_XCVR); // 11b IDs, 250k bit rate, no transceiver chip, portA pins 11,12
...
void loop()
{
  if (millis() / txDly != last)             // tx every txDly
  {
    ...
    if ( digitalRead( bluePillLED) == LOW) {
      digitalWrite(bluePillLED, HIGH); }
    else {
      digitalWrite(bluePillLED, LOW); }
  }
  /*
